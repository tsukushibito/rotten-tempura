...
class TestRenderer {
 public:
  TestRenderer(const std::shared_ptr<gfx::Device>& device) {
    ...
    // pipeline_layout_ =
    // vk_device.createPipelineLayoutUnique(pipeline_layout_ci);
    ...
    auto&& render_pass = tvk_swap_chain->render_pass();
    ...
    for (int i = 0; i < command_buffers_.size(); ++i) {
      ...
      render_pass_bi.renderPass = tvk_swap_chain->render_pass();
      ...
      render_pass_bi.framebuffer = tvk_swap_chain->frame_buffer(i);
...
int main(int argc, char* argv[]) {
  ...
  auto dataManager = temp::ObjectManager<Data>::Create();
  {
  ...
  TEMP_LOG_TRACE("Befor RemoveUnusedObjects");
  dataManager->RemoveUnusedObjects();
  TEMP_LOG_TRACE("After RemoveUnusedObjects");
